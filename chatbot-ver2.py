...
print('Welcome To Sup.AI!')

while True:
    prompt = input("Ask a question:\n")

    # Convert the prompt string to a tuple of characters
    prompt_tuple = tuple(prompt)

    # Debugging: Print the type of the input to encode
    print(f"Type of input to encode: {type(prompt_tuple)}")

    # Pass the tuple to the encode function
    context = torch.tensor(encode(prompt_tuple), dtype=torch.long, device=device)

    generated_output = model.generate(context.unsqueeze(0), max_new_tokens=150)
    generated_indices = generated_output[0].tolist()
    print(f"Generated indices: {generated_indices}")

    generated_chars = decode(tuple(generated_indices))
    print(f'Answer:\n{generated_chars}')
